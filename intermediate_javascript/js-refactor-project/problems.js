...
function countConsonants(word) {
    const vowels = ["a", "e", "i", "o", "u"];
    let count = 0;
    const wordModified = word.replace(" ", "");
    for (let i = 0; i < wordModified.length; i++) {
        if (!vowels.includes(wordModified[i])) {
            count++;
        }
    }
    return count;
...
function alternatingLetters(str) {
    return str.split('').map((letter, index) => index % 2 === 0 ? letter.toLowerCase() : letter.toUpperCase()).join('');
}
